...
function Contributors() {
  ...
  const [Alladmindata, setadmindata] = useState([]);
  const [loader, setLoader] = useState(false);
  const alldata = useSelector(
    ({ ServiceProviderAgent }) => ServiceProviderAgent?.admindata
  );
  const loading = useSelector(
    ({ ServiceProviderAgent }) => ServiceProviderAgent?.loader
  );
  const getAlladminData = (payload) => dispatch(onAdminGet(payload));
  useEffect(() => {
    console.log("Admindata", alldata);
    if (alldata) {
