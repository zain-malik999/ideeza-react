...
function AgreementTabsHeader(props) {
  const { title, icon } = props;
  return (
    <div className="flex flex-wrap justify-between">
      <div className="flex items-center lg:px-4 px-2 space-x-3 pb-3 border-b-2 border-ideeza-100 text-ideeza-100">
        {/* <i className="el-icon-document lg:text-3xl text-2xl pr-3"></i> */}
        {/* <img src={icon} alt="" /> */}
        {icon}
        <h2 className="lg:text-xl text-lg font-sans">{title}</h2>
      </div>
      <div className="flex pt-4 lg:pt-0 lg:space-x-2 lg:w-auto w-full justify-end">
        <div className="text-sm text-gray-600 flex items-center">
          ...
          <span className="md:px-3 px-1 lg:pr-10">
            <select className="text-xs md:text-sm bg-white focus:outline-none focus:shadow-outline">
        ...
        <div className="text-xs md:text-sm text-gray-600 flex items-center">
          ...
          <span className="md:pl-3 pl-1">Download PDF</span>
        </div>
      </div>
    </div>
  );
